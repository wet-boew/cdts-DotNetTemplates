        ...
        public void UpdateCulture(string culture)
        {
            if (string.IsNullOrEmpty(culture))
            {
                if (_sessionController.SessionExists())
                {
                    ...
                    culture = Convert.ToString(_sessionController.Get(Constants.SESSION_CULTURE_KEY), CultureInfo.CurrentCulture);

                    if (string.IsNullOrEmpty(culture))
                    {
                        //culture not found in session, use default language
                        culture = Constants.ENGLISH_CULTURE;
                        _sessionController.Set(Constants.SESSION_CULTURE_KEY, culture);
                    }
                }
                else
                {
